...
ylabel('x (Amplitude)');
axis([0 1000 -2 8])
plotcmds(sprintf('./figures/sin_blip_run_%d',run));
...
axis([0 0.5 0 14])
plotcmds(sprintf('./figures/sin_blip_periodogram_run_%d',run));
...
axis([0 0.03 0 2])
legend('All data','x(401:500) only')
xlabel('frequency [units of 1/\Delta t]');
ylabel('Raw periodogram');
plotcmds(sprintf('./figures/sin_blip_periodogram_zoom_run_%d',run));
...
xlabel('t (time step)');
colorbar;
plotcmds(sprintf('./figures/sin_blip_spectrogram_run_%d',run));
...
plotcmds(sprintf('./figures/sin_blip_spectrogram_histogram_run_%d',run));
end
